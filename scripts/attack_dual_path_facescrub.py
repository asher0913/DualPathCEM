...
sys.path.insert(0, str(ROOT / "third_party" / "legacy_slot_cem" / "src"))
...
def load_feature_extractors(checkpoint_path: Path, device: torch.device):
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    args = checkpoint["args"]
    legacy_dir = Path(checkpoint["legacy_checkpoint_dir"])
    legacy = vgg11_bn_sgm(
        4,
        SilentLogger(),
        num_client=1,
...
@torch.no_grad()
def evaluate(
    generator_model: DualPathReconstructor,
    legacy: nn.Module,
    semantic: GlobalSemanticBottleneck,
    loader: DataLoader,
    device: torch.device,
    ...
    for images, _labels in loader:
        ...
        reconstructed = generator_model(spatial, token)
...
def evaluate_over_noise_seeds(
    ...
    aggregate = {
        name: float(np.mean([float(run[name]) for run in runs]))
...
def main() -> None:
    ...
    train_loader, evaluation_loader = build_attack_loaders(
        args.data_root, args.batch_size, args.workers, args.attack_knowledge
    )
    legacy, semantic, legacy_noise, semantic_noise, token_dim = (
        load_feature_extractors(args.dual_path_checkpoint, device)
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, _labels in train_loader:
            ...
            reconstructed = reconstructor(spatial, token)
            ...
            if discriminator is not None:
                ...
                generator_adv = adversarial_criterion(
    ...
    result = {
        ...
        "attacker_observes_spatial_and_semantic_paths": True,
        ...
        "perceptual_evaluator": "LPIPS-Alex"
        if args.perceptual_metrics
        else None,
    }
