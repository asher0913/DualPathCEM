...
def test_aggregate_results_has_no_manuscript_finalizer(monkeypatch, tmp_path) -> None:
    commands = {}
    ...
    pipeline.aggregate_results({}, tmp_path / "state.json", 3, 0.0)
    assert set(commands) == {"aggregate_supplementary"}
    assert "scripts/aggregate_dual_path_supplementary.py" in commands[
